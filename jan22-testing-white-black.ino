...
#define TRIG_PIN 10
#define ECHO_PIN 11
#define IR_PIN 12   // LM393 IR sensor (LOW = BLACK, HIGH = WHITE)
...
// ===== IR SENSOR =====
// TRUE when on WHITE line (danger)
bool onWhite() {
...
void loop() {
  ...
  if (digitalRead(IR_PIN) == HIGH) {   // WHITE detected
  Serial.println("IR: WHITE | MODE: EDGE OVERRIDE → BACKWARD");
  backward(MAX_SPEED);
  delay(120);
  search();       // spin away from edge
  delay(150);
  return;
  } 
  ...
  Serial.print(" | IR: ");
  Serial.print(onWhite() ? "WHITE" : "BLACK");

  // ===== BEHAVIOR PRIORITY =====
  if (onWhite()) {
    Serial.print(" | MODE: EDGE → BACKWARD");
    backward(MAX_SPEED);
  }
